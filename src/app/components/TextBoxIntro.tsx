...
export function TextBoxIntro({ content = "", title, image, bgColor, textColor, imagesubtitle, link, hide = false }: textInfoProp) {
 ...
 const { setIsImgActive } = useMapStore();
  ...
  return (
    <div id="container" className=" flex flex-col w-[full]  ">
      ...
      {
        content && (
          <div id="box-container" className={`px-5 py-7 bg-gradient-to-r from-sand-300 to-sand-100   ${bgColor ? bgColor : 'bg-[#fff]'}`}>
            <div className={`h-80 sm:h-36 md:h-36 text-base ${textColor ? textColor : 'text-[#BB7843]'} font-medium`}>
              <AnimatedText text={content} limit={130} />
            </div>
              {link && (
